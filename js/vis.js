...
vistk.viz = function() {
  ...
  if (vars.dev) console.log("Init")
			...
			if(level == 0) {
			  ...
			  nested_data = d3.nest()
					...
					.rollup(function(leaves) { 
			  		...
			  		if(item_type == "product") {
							return {
								"opp_gain": d3.mean(leaves, function(d) { return parseFloat(d.opp_gain);}), 
								"pci": d3.mean(leaves, function(d) { return parseFloat(d.pci);}),
								"rca": d3.mean(leaves, function(d) { return parseFloat(d.rca);}),
								"value": d3.sum(leaves, function(d) { return parseFloat(d.value);})}
						} else {

							return {
								"year": d3.mean(leaves, function(d) { return parseFloat(d.year);}),
								"id": leaves[0].region_id,
								"name": leaves[0].continent,
								"continent": leaves[0].continent,
								"abbrv": leaves[0].abbrv
							}
}
							})
    ...
    nested_data = nested_data
				...
				nested_data = vars.data;

			}

			d3.select("#viz").select("table").remove();

		  vars.table = vars.parent.selectAll("table").data([nested_data.filter(function(d) { 
		  	...
		  	if(vars.solo.length > 0) {
		  		...
		  		if(item_type == "product")
						...
						return (d.year == vars.year) && (d.continent == vars.solo[0]);
				    ...
				    .data(function(d) {
				    	...
				    	return vars.columns.map(function(c) { 
					        ...
					        if(item_type=="country")
					          return '<a href="/country/'+d.abbrv+'/"><img src="http://atlas.cid.harvard.edu/media/img/icons/flag_'+d.abbrv.toLowerCase()+'.png" style="width: 20px;">'+d[c]+'</a>';
					        else
